 ...
 * @version 1.0
 * @date 2019-02-12
 * @brief CMSIS Peripheral Access Layer for QN9090
 *
 * CMSIS Peripheral Access Layer for QN9090
 */

#ifndef _QN9090_H_
#define _QN9090_H_                               /**< Symbol preventing repeated inclusion */
...
/*! COPY_SKEY - Copies Secret Key and Enables Cipher
 */
...
/*! ble_ant_selected - Selection of antenna. 0: ADE is asserted. 1: ADE de-asserted. ADO is always
 *    the inverse of ADE and so is also controlled by this setting as well.
 */
...
typedef struct {
  ...
  __IO uint32_t DMA_ITRIG_INMUX[19];               /**< Trigger select register for DMA channel, array offset: 0xE0, array step: 0x4 */
 ...
 *    interrupt; 1 = ADC0 Sequence B interrupt; 2 = Timer CT32B0 Match 0; 3 = Timer CT32B0 Match 1;
 *    4 = Timer CT32B1 Match 0; 5 = Timer CT32B1 Match 1; 6 = Pin interrupt 0; 7 = Pin interrupt 1;
 *    8 = Pin interrupt 2; 9 = Pin interrupt 3; 10 = AES RX; 11 = AES TX; 12 = Hash RX; 13 = Hash
 *    TX; 14 = DMA output trigger mux 0; 15 = DMA output trigger mux 1; 16 = DMA output trigger mux 2;
 ...
 *    (must be enabled in functional mux); 1 = XTAL 32 MHz (must be enabled in clock_ctrl); 2 = FRO
 ...
 *    configured as GPIO); 7 = PIO[16] (must be configured as GPIO); 8 = PIO[15] (must be configured
 *    as GPIO).
 */
 ...
 *    CLK_IN (must be enabled in functional mux); 1 = XTAL 32 MHz (must be enabled in clock_ctrl); 2 =
 *    FRO 1 MHz (must be enabled in clock_ctrl); 3 = 32 kHz oscillator (either FRO 32 KHz or XTAL 32
 *    KHZ); 4 = Main clock (divided); 5 = PIO[4] (must be configured as GPIO); 6 = PIO[20] (must be
 *    configured as GPIO); 7 = PIO[16] (must be configured as GPIO); 8 = PIO[15] (must be
 *    configured as GPIO).
 */
...
typedef struct {
  __IO uint32_t PIO[1][22];                        /**< Digital I/O control for port 0 pins PIO0_0..Digital I/O control for port 0 pins PIO0_21, array offset: 0x0, array step: index*0x58, index2*0x4 */
} IOCON_Type;
...
/*! @name PIO - Digital I/O control for port 0 pins PIO0_0..Digital I/O control for port 0 pins PIO0_21 */
/*! @{ */
...
/*! MODE - Select function mode (on-chip pull-up/pull-down resistor control).
...
/*! ECS - Pull-up current source enable when set.
...
/*! SLEW0 - Driver Slew Rate. This bit field is used in combination with SLEW1. The higher
 *    [SLEW1,SLEW0] the quicker the IO cell slew rate.
 *  0b0..Disabled. Driver slew rate disabled.
 *  0b1..Enabled. Driver slew rate enabled.
 */
...
/*! FILTEROFF - Control Input Glitch Filter.
 ...
 *  0b0..Noise pulses below approximately 50 ns are filtered out.
 ...
 *  0b0..Disabled. IO clamping function disabled.
 *  0b1..Enabled. IO clamping function enabled.
 */
...
/*! IO_CLAMP - Assert to freeze the IO. Also needs SYSCON_RETENTIONCTRL set as well. Useful in power
 *    down mode. This mode is held through power down cycle. Before releasing this mode on a
 *    wake-up, ensure the IO is set to the required direction and value by using GPIO_DIR and GPIO_PIN
 *    registers.
 *  0b0..Disabled. IO clamping function disabled.
 *  0b1..Enabled. IO clamping function enabled.
 */
