...
#define BUILD_ASSERT_FSL_SHA(condition, msg) extern int msg[1 - 2 * (!(condition))] __attribute__((unused))
...
status_t SHA_Init(SHA_Type *base, sha_ctx_t *ctx, sha_algo_t algo)
{
    ...
    BUILD_ASSERT_FSL_SHA(sizeof(sha_ctx_t) >= sizeof(sha_ctx_internal_t), sha_ctx_t_size);
    uint32_t i;
