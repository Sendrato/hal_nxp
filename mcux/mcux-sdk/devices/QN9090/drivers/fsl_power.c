...
bool POWER_BodVbatConfig(pm_bod_cfg_t *bod_cfg_p)
{
    ...
    // printk( "BODVBAT=0x%X ANACTRL_CTRL=0x%X _STAT=0x%X _VAL=0x%X _INTENSET=0x%X\n", PMC->BODVBAT,
...
#ifdef TRACE_VRB
    printk("POWER_EnterPowerDownMode:\n");
    printk("  wakeup_src0      : 0x%x\n", wakeup_src0);
    printk("  wakeup_src1      : 0x%x\n", wakeup_src1);
    printk("  wakeup_io        : 0x%x\n", pm_power_config->pm_wakeup_io);
    printk("  pm_config        : 0x%x\n", pm_power_config->pm_config);
#endif
...
#ifdef TRACE_VRB
    printk("POWER_EnterDeepDownMode:\n");
    printk("  wakeup_src0      : 0x%x\n", wakeup_src0);
    printk("  wakeup_src1      : 0x%x\n", wakeup_src1);
#else
    (void)wakeup_src0;
