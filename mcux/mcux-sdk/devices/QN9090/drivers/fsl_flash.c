...
int FLASH_Program(FLASH_Type *pFLASH, uint32_t *pu32Start, uint32_t *pu32Data, uint32_t u32Length)
{
    ...
    uint32_t start = ((uint32_t)pu32Start) - (((uint32_t)pu32Start) % FLASH_PAGE_SIZE);
    uint32_t end   = (uint32_t)pu32Start + u32Length;

    uint32_t padding_start = (uint32_t)pu32Start - start;
    uint32_t padding_end   = (FLASH_PAGE_SIZE - (end & (FLASH_PAGE_SIZE-1))) & (FLASH_PAGE_SIZE-1);

    pFLASH->INT_CLR_STATUS = FLASH_STAT_ALL;

    pFLASH->AUTOPROG = FLASH_AUTO_PAGE;

    /* padding when data is not aligned with start of page */
    ...
    /* padding when end of data does not align with end of page */
    while (padding_end-- > 0) {
        *(uint8_t*)end ++ = 0;
    }
